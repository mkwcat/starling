...
USB::USBError USB::GetDeviceInfo(u32 devId, DeviceInfo* outInfo, u8 alt)
{
    ...
    WriteU32(input, devId);
    WriteU8(input + 0x8, alt);
...
USB::USBError USB::Attach(u32 devId)
{
    ...
    s32 ret = m_ven.Ioctl(USBv5Ioctl::Attach, input, 32, nullptr, 0);
...
USB::USBError USB::SuspendResume(u32 devId, State state)
{
    u8* input = (u8*) IOS::Alloc(32);
    WriteU32(input, devId);
    WriteU8(input + 0xB, state == State::Resume ? 1 : 0);

    s32 ret = m_ven.Ioctl(USBv5Ioctl::SuspendResume, input, 32, nullptr, 0);
...
USB::USBError USB::CancelEndpoint(u32 devId, u8 endpoint)
{
    ...
    WriteU32(input, devId);
    WriteU8(input + 0x8, endpoint);
    s32 ret = m_ven.Ioctl(USBv5Ioctl::CancelEndpoint, input, 32, nullptr, 0);
  ...
  u16 index, u16 length, void* data)
{
    ...
    if ((requestType & CtrlType::Dir_Mask) == CtrlType::Dir_Host2Device) {
        ...
        vec.in[1].data = data;
        vec.in[1].len = length;
        ret = m_ven.Ioctlv(USBv5Ioctl::CtrlTransfer, vec);
    } else {
