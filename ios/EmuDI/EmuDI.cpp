...
static inline void ReqIoctl(IOS::Request* req)
{
    ...
    // Real drive
    const s32 ret = IOS_Ioctl(DI::Instance()->GetFd(), req->ioctl.cmd,
      req->ioctl.in, req->ioctl.in_len, req->ioctl.out, req->ioctl.out_len);
    req->Reply(ret);
}
...
static inline void ReqIoctlv(IOS::Request* req)
{
    ...
    // Real drive
    const s32 ret = IOS_Ioctlv(DI::Instance()->GetFd(), req->ioctlv.cmd,
