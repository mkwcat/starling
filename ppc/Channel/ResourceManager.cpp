...
{
    if (s_initInfo.tmd.numContents <= index) {
        return IOS::IOSError::INVALID;
    }

    if (s_initInfo.tmd.contents[index].type == ES::TMDContent::Type::NORMAL) {
        char path[64];
        u64 titleID = s_initInfo.tmd.titleId;

        snprintf(
            path, 64, "/title/%08x/%08x/content/%08x.app", U64Hi(titleID),
            U64Lo(titleID), s_initInfo.tmd.contents[index].cid
        );

        return IOS_Open(path, IOS::Mode::READ);
    }

    if (s_initInfo.tmd.contents[index].type != ES::TMDContent::Type::SHARED) {
        return IOS::IOSError::INVALID;
    }
    ...
    auto entry = std::find_if(
        ...
        return memcmp( //
                   entry.hash, s_initInfo.tmd.contents[index].hash, 0x14
               ) == 0;
        });
