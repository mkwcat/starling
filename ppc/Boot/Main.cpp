...
#include <Boot/ICache.hpp>
#include <Boot/Init.hpp>
#include <Debug/Log.hpp>
#include <Debug/Console.hpp>
#include <Patcher/Arguments.hpp>
...
ChannelInitInfo* GetInitInfo()
{
    s_initInfo = {
        .twmTable = &TwmTable,
        .twmTableEnd = &TwmTableEnd,
...
void Launch()
{
    ...
    Boot_IOS::SafeFlush(reinterpret_cast<void*>(IOS_BOOT_ADDRESS), file->size);
    ...
    Boot_IOS::SafeFlush(s_bootArcData + file->offset, file->size);

    if (!Boot_IOS::BootstrapEntry()) {
    ...
    Patcher::Arguments::ParseCommandLine(argc, argv);
    ...
    ChannelInitInfo* info = GetInitInfo();

    s32 importCount = std::distance(info->twmTable, info->twmTableEnd);

    for (s32 i = 0; i < importCount; i++) {
        u32 address = info->twmTable[i].address;

        switch (info->twmTable[i].type) {
        case Twm_FImport:
            // Patch the stub function to a branch
            WriteU32(
                info->twmTable[i].stub | 0xC0000000,
                0x48000000 |
                    ((address - u32(info->twmTable[i].stub)) & 0x03FFFFFC)
            );
            break;
        ...
        case Twm_FReplace:
            ...
            WriteU32(
                address | 0xC0000000,
                0x48000000 |
                    ((u32(info->twmTable[i].stub) - address) & 0x03FFFFFC)
            );
            break;

        case Twm_DImport:
            // Write the offset to the stub address
            WriteU32(info->twmTable[i].stub, address);
            break;
        }
    }

    std::memcpy(&info->tmd, &s_shopTmd, sizeof(info->tmd));
    info->rgnselVer = s_rgnselVer;
