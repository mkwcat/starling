...
DI::DI(const char* path)
{
    PRINT(DVD, ERROR, "Attemping open DI (path: \"%s\")", path);

    for (u32 i = 0; i < 16; i++) {
        new (&m_di) IOS::ResourceCtrl<DIIoctl>(path, 0);
        if (m_di.GetFd() != IOS::IOSError::NOT_FOUND) {
            break;
        }
    }
    ...
    if (m_di.GetFd() < 0) {
        PRINT(
            DVD, ERROR, "Failed to open DI (path: \"%s\"): %d", path,
            m_di.GetFd()
        );
    }
}
...
DI::DIError DI::ReadDiskID(DiskID* diskid)
{
    ...
    DIError res =
        CallIoctl(block, DIIoctl::ReadDiskID, &drvDiskid, sizeof(DiskID));
    if (res == DIError::OK)
        *diskid = drvDiskid;
    return res;
}
...
DI::DIError DI::GetLength(u32* length)
{
    ...
    u32 drvLength;
    DIError res = CallIoctl(block, DIIoctl::GetLength, &drvLength, sizeof(u32));
    *length = drvLength;
    return res;
}
...
{
    if (!IsAligned(tmd, 32) || !IsAligned(ticket, 32) || !IsAligned(certs, 32))
    ...
    DIError res =
        static_cast<DIError>(m_di.Ioctlv(DIIoctl::OpenPartition, vec));
    if (esError != nullptr)
...
)
{
    ...
    DIError res = static_cast<DIError>(
        m_di.Ioctlv(DIIoctl::OpenPartitionWithTmdAndTicket, vec)
    );
    if (esError != nullptr)
        *esError = static_cast<ES::ESError>(output[0]);
...
)
{
    ...
    DIError res = static_cast<DIError>(
        m_di.Ioctlv(DIIoctl::OpenPartitionWithTmdAndTicketView, vec)
    );
    if (esError != nullptr)
        *esError = static_cast<ES::ESError>(output[0]);
...
DI::DIError DI::CallIoctl(DICommand& block, DIIoctl cmd, void* out, u32 outLen)
{
    return static_cast<DIError>(
        m_di.Ioctl(cmd, &block, sizeof(DICommand), out, outLen)
    );
}
