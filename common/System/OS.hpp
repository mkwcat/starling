...
public:
    ...
    {
        T msg;
        const s32 ret = IOS_ReceiveMessage(this->m_queue, (u32*) (&msg), 1);
        assert(ret == IOS_ERROR_OK);
        return reinterpret_cast<T>(msg);
    }
...
public:
     ...
     */
    PPC_Thread(Proc proc, void* arg = nullptr, u8* stack = nullptr,
     ...
     * Call IsValid() to check if the thread was successfully created.
     */
    void Create(Proc proc, void* arg = nullptr, u8* stack = nullptr,
      u32 stackSize = 0x8000, s32 prio = 10, bool autoRun = true)
    {
        ...
        if (!OSCreateThread(&m_thread, ThreadProc,
              reinterpret_cast<void*>(this), stackTop, stackSize, prio, 0)) {
            return;
        }
